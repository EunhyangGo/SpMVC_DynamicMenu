...
public class MenuService {
	...
	public List<DropMenuDto> getDBMenu(){
		List<DropMenuDto> menuList = new ArrayList<DropMenuDto>();
		
		return menuList;
	}
	
	public List<MenuDto> getBuiler(){
		List<MenuDto> menuList = new ArrayList<MenuDto>();
		
		// 생성자와 setter의 장점을 모두 보유한 방식
		// 생성자패턴 : 다량의 데이터를 setting해야할 경우 생성자에 직접 데이터를 주입하면서
		//			  dto(vo)를 만들 수 있다.
		// 		단점: 데이터를 주입할 때 항상 순서에 신경을 써야 한다. 
		//			  만약 순서를 제대로 지키지 않으면 데이터가 엉뚱하게 사용될 수 있다.
		// 생성자 패턴의 그러한 단점을 보완하기 위해서 setter 패턴을 사용한다.
		//   setter 패턴의 가장 큰 단점 : 코드가 길어지고, 난해한(읽기 힘든) 코드가 될 수 있다.
		
		// Builder패턴은 생성자호출을 하면서 변수이름을 직접 보고
		// 데이터를 추가할 수 있기때문에 순서가 바뀌거나 하는 경우가 없고,
		// setter 패턴처럼 코드가 번거러워지는 현상이 없다.
		MenuDto m = MenuDto
				.builder() // 지금부터 builder 패턴 방식으로 시작
				.menu_id("M0001")
				.menu_title("홈으로")
				.menu_href("/").build();
		...
		menuList.add(MenuDto.builder()
				.menu_id("M0002").menu_title("치킨나라").menu_href("CH1").build());
		
		menuList.add(MenuDto.builder()
				.menu_id("M0003").menu_title("치킨나라").menu_href("CH1").build());
		
		menuList.add(MenuDto.builder()
				.menu_id("M0004").menu_title("치킨나라").menu_href("CH1").build());
		return menuList;
	}
